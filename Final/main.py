...
########## Player ##########
    # Player animation moving left and right
        for i in range(1, 8):
            locx = i*32
            locy = 64
            self.player_left = self.player.get_image(locx, locy, 32, 32)
...
########## Enemy ##########
        for i in range(0, 4):
            locx =  i * 24
            locy = 0
            self.left_e = self.slime.get_image(locx, locy, 24, 24) # left/right
...
########## Enemy ##########
    ...
    def new(self):
        ...
        self.map = pytmx.load_pygame(map_list[current_level])
                ...
                for obj in layer:
                    if obj.name == 'Player':
                        self.guy = Player(self.screen, self.player_right_list, self.player_left_list, 
                                          ...
                                          self.attack_list, self.hit_list, self)
                      ...
                      enemy_list.append(self.enemy)                    
                        ...
                        self.level = Object(self.screen, obj.x*mult, obj.y*mult, self.coin_list, self)
